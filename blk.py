...
def create_blk_fund_info() -> pd.DataFrame:
    def parse_cookies(cookie_string: str) -> str:
        return str({pair.split('=')[0]: pair.split('=')[1] for pair in cookie_string.split('; ')})

    def get_aladdian_portfolio_ids() -> List[str]:
        ids_url = "https://www.ishares.com/us/product-screener/product-screener-v3.1.jsn?dcrPath=/templatedata/config/product-screener-v3/data/en/us-ishares/ishares-product-screener-backend-config&siteEntryPassthrough=true"
        headers = {
            "authority": "www.ishares.com",
            "method": "GET",
            "path": "/us/product-screener/product-screener-v3.1.jsn?dcrPath=/templatedata/config/product-screener-v3/data/en/us-ishares/ishares-product-screener-backend-config&siteEntryPassthrough=true",
            "scheme": "https",
    ...
    def get_raw_xls_etf_info() -> bytes:
        ...
        headers = {
            "authority": "www.ishares.com",
            "method": "POST",
            "path": "/us/product-screener/product-screener-v3.1.jsn?type=excel&siteEntryPassthrough=true&dcrPath=/templatedata/config/product-screener-v3/data/en/us-ishares/ishares-product-screener-excel-config&disclosureContentDcrPath=/templatedata/content/article/data/en/us-ishares/DEFAULT/product-screener-all-disclaimer",
            "scheme": "https",
            "Accept": "application/json, text/plain, */*",
            "Accept-Encoding": "gzip, deflate, br",
            "Accept-Language": "en-US,en;q=0.9",
        ...
        res = requests.post(xls_url, data=payload,
    ...
    df_with_cols.to_excel(f"{curr_date}_blk_fund_info.xlsx", index=False, sheet_name=f"{curr_date}_blk_fund_info.xlsx")
